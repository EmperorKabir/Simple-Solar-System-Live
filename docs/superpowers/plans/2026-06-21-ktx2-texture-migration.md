...
- [ ] **Step 5: Skip `labelRenderer.render()` when idle, with a heartbeat** — first hoist the camera-delta computation OUT of the SLSS diag block (3358-3361) so it runs always-on. Skip the CSS2D render (3444) on a frame ONLY when ALL of: camera static (`posDelta≈0 && tgtDelta≈0`); no time-nudge button pressed/held and no jump in progress; and the heartbeat is not due. RUN the render when ANY of: camera moving; a time-nudge button is pressed/held or a jump just fired; or the ~2/second heartbeat fires (`frameCount % 30 === 0`). The heartbeat bounds any live real-time orbital drift to a fraction of a second of motion (imperceptible), so labels can never visibly detach — and this holds whether or not a true paused state exists. The overlap pass (3510) reads `getBoundingClientRect`, so on any frame the render is skipped the overlap pass must also be skipped that frame (it already runs on its own interval; gate it on "render ran this frame OR camera/ time idle since last overlap"). Commit.
