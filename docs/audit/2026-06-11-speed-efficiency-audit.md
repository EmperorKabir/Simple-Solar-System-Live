...
- **D2 — RESOLVED: REMOVED.** On-device A/B on a physical Galaxy Z Fold 6 (SM-F966B): **no fold/unfold flicker with OR without** the forced hardware layer → it was redundant (WebView is HW-accelerated by default; the layer added a per-frame full-screen composite). Removed.

## Stage-4 on-device evidence — 2026-06-14 (Galaxy Z Fold 6, SM-F966B, diag build `d235290`)
- **No-regression / cold launch:** SLSS log over **8,613 events, 0 `error` events**; **8,179 frame_trace** (main loop incl. the A1 moon path) median **8.4 ms** / p90 19.6 ms; 190 texture_loads; GPU healthy. Screencap: full solar system renders correctly (all planets, rings, labels, live clock).
- **Render pipeline (B1/C1/B2):** 4 complete widget renders through every stage (`worker_start → renderer_construct → js_snapshot_ready → compose_done → callback_returned`), all `bitmap_null=false`, VDs created+destroyed cleanly. Correctly tagged `widget_render` → confirms B2's `startsWith("widget")` + the off-main compose (C1) + atomic resolution (B1).
- **D3 (alpha:false main):** screencap shows pure-black background, no transparency artifacts — visual parity confirmed.
- **UI fixes during the session (user-requested):** time-nudge buttons → 6-col grid with each unit's − and + column-aligned (−1h above +1h …). Font "looks bolder unfolded" = `clamp(…vmin…)` size scaling, not weight (no change). Pluto-off-centre-when-shown = pre-existing Sun-centred framing (my diff never touches `calcResetView`) → left as-is per user.
### Full on-device pass — 2026-06-14 (all previously-flagged scenarios now EMPIRICALLY confirmed)
- **B2 multi-widget collapse — PASS.** With 5 widgets placed (ids 75–79), a fold-refresh burst (induced via `cmd device_state` 0→3) re-rendered **all 5 simultaneously: 13 fresh renders, every one successful, 0 null/collapsed**. The old constant-`"widget"` key would have collapsed the queued ones to null; the per-id `"widget-$id"` key does not. (The protected `APPWIDGET_UPDATE` broadcast is adb-denied, so the fold-refresh path — enabled once the wallpaper engine is bound — was the trigger.)
- **C2 wallpaper encode — PASS.** Home wallpaper set to `SolarSystemHomeWallpaperService`; **7 complete `wallpaper_render` cycles** through every stage; the background `diskExecutor` wrote `wallpaper_wallpaper_home_2184x1968.webp` (41 KB, fresh) — the off-main WebP encode runs and completes.
- **B3 lock staleness — PASS.** hide → stale (15 s throwaway test interval) → show fired an age-gate render **+95 ms** after becoming visible, with **`ms_since_last_render=33074`** (>interval) and a successful completion — the stale-on-visible re-render, not the periodic timer or a param/dim change. Production interval restored to 10 min; final build reinstalled.
- **Session totals:** 0 `error` events across every log; the simulated fold state was reset; the device runs the production-interval build.
- **Verdict:** A1, B1, B2, B3, C1, C2, A3, D1, D2, D3 — all implemented AND on-device verified on a physical Galaxy Z Fold 6.
