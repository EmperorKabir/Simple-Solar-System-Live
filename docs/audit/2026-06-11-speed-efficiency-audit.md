...
- **Not exercised in-session (honest gaps):** B2 multi-*instance* collapse (needs ≥2 widgets placed); wallpaper `wallpaper_render`/C2 (needs the live wallpaper set); B3 lock-staleness (needs a >10-min stale interval). All three are structurally verified by build + code; empirical multi-widget/wallpaper proof pending optional further setup.
