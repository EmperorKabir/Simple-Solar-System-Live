...
class SurfaceSettings(
    ...
    var moonLabelsEnabled: Boolean
        get() = prefs.getBoolean("moonLabels", DEFAULT_MOON_LABELS)
        set(value) = prefs.edit().putBoolean("moonLabels", value).apply()
    ...
    fun urlParams(surface: String): String {
        ...
        val mLabels = if (moonLabelsEnabled) "on" else "off"
        return "?surface=$surface&offsetY=$offset&tilt=$tiltR&labels=$labels&pluto=$pluto&moonLabels=$mLabels"
    }
    ...
    companion object {
        ...
        const val DEFAULT_MOON_LABELS = false
